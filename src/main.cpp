...
#if defined(NDEBUG)
# error "Millibyte cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe3c567c3607563c77cd9d3a2ff62f68832b89f04209d4aaeade2f83aaf0b3f80");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Millibyte: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Millibyte Signed Message:\n";
...
{
    // Millibyte: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Millibyte: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Millibyte: 5 minutes
static const int64 nTargetSpacing = 15; // Millibyte: 15 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Millibyte: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Millibyte: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe3c567c3607563c77cd9d3a2ff62f68832b89f04209d4aaeade2f83aaf0b3f80");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NBC News 03/Jan/2017 Rape Allegation Against Harvey Weinstein is 'Credible', Police Official Says";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5123456780f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1509755911;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4661136;
        ...
        assert(block.hashMerkleRoot == uint256("0x15c3a4eb6c574161161dd0eeb103346795ccc6b8d7163635482a4f809260f59e"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Millibyte: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MillibyteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MillibyteMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MillibyteMiner(CWallet *pwallet)
{
    printf("MillibyteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("millibyte-miner");
    ...
    try { loop {
        ...
        printf("Running MillibyteMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MillibyteMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MillibyteMiner, pwallet));
}
